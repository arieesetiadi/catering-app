...
$query ="SELECT title FROM tags WHERE title ='$title' and type='1'";
$result= $connection->query($query) or die($connection->error.__LINE__);
$hasil= $result->num_rows;
if ($hasil== 0){
  
$tambah= "INSERT INTO tags(title,seotitle,type) values('$title', '$seotitle', '1')" or die($connection->error.__LINE__); 
...
// edit
elseif ($modul=='tags' AND $aksi=='edit'){
 $error = array();
...
if (empty($error)) { 
$update="UPDATE tags SET title='$title',seotitle='$seotitle' WHERE tags_id='$id'" or die($connection->error.__LINE__); 
...
if ($aksi=='delete'){
$delete="DELETE FROM tags WHERE tags_id='$id'" or die($connection->error.__LINE__); 
if($connection->query($delete) === true) { 
_goto( '../../?mod=tags'); // sukses
} else { 
_goto( '../../?mod=tags');
...
elseif ($modul=='tags' AND $aksi=='multidelete'){
   $idArr = $_POST['item'];
          foreach($idArr as $id){
          mysqli_query($connection,"DELETE FROM tags WHERE tags_id=".$id);
  }
 _goto( '../../?mod='.$modul.'');
}
