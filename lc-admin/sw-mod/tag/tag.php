...
$query_role = "SELECT read_access,write_access,modify_access,delete_access FROM user_role where module_id='11' AND level_id='$level_user'"; 
...
<tbody>';
$query="SELECT id,title,seotitle from tags where type='2' order by tags.id DESC";
...
<input class="multicheck" type="checkbox" name="item[]" value="'.$id.'">
</div>
</td>
<td>'.$title.'</td>
<td>'.$seotitle.'</td>
...
<a href="#edit_form" class="btn btn-xs btn-default enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('title').value='<?PHP echo $title;?>';getElementById('id').value='<?PHP echo $id;?>';"><i class="fa fa-pencil"></i></a>
<?PHP if($delete_access == 'Y'){
  echo'
<a href="javascript:void(0)" data-href="'.$gotoprocess.'?id='.$id.'&aksi=delete" data-toggle="modal" data-target="#hapus" class="btn btn-xs btn-danger enable-tooltip" title="'._e('Delete').'"><i class="fa fa-trash-o"></i></a>';}
else {
    echo'
