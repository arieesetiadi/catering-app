...
<i class="fa fa-tags"></i>'._e('Category Management').'<br></h1>
</div></div>
<ul class="breadcrumb breadcrumb-top">
<li><a href="./"><i class="fa fa-home"></i> '._e('Dashboard').'</a></li>
<li>'._e('Category Management').'</li>
</ul>';
...
<tr>
  ...
  <th>'._e('Title').'</th>
  <th>'._e('Title Seo').'</th>
  <th width="8%">'._e('Action').'</th>
</tr>
</thead>
<tbody>';
$query="SELECT category_id,title,seotitle from category where type='2' order by category.category_id DESC";
...
<input class="multicheck" type="checkbox" name="item[]" value="'.$category_id.'">
</div>
</td>
<td>'.$title.'</td>
<td>'.$seotitle.'</td>
...
<a href="#edit_form" class="btn btn-xs btn-default enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('title').value='<?PHP echo $title;?>';getElementById('id').value='<?PHP echo $category_id;?>';"><i class="fa fa-pencil"></i></a>
<?PHP if($delete_access == 'Y'){
  echo'
<a href="javascript:void(0)" data-href="'.$gotoprocess.'?id='.$category_id.'&aksi=delete" data-toggle="modal" data-target="#hapus" class="btn btn-xs btn-danger enable-tooltip" title="'._e('Delete').'"><i class="fa fa-trash-o"></i></a>';} else {
  echo'
        ...
        <div class="modal-content">
          <form id="validate" class="form" method="post" action="'.$gotoprocess.'">
            <div class="modal-header">
              ...
              <h3 class="modal-title">'._e('Add New Category').'</h3>
            </div>
            <div class="modal-body">
              <input type="hidden" name="modul" value="'.$mod.'" readonly>
              ...
              <div class="form-group">
                ...
                <input class="form-control" type="text" name="title" placeholder="Kategori baru" required>
              </div>
            </div>
            <div class="modal-footer">
      ...
      <div class="modal-dialog modal-sm">
        <div class="modal-content">
          <form id="validate" method="post" action="'.$gotoprocess.'">
            <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              <h3 class="modal-title">'._e('Edit Category').'</h3>
            </div>
            <div class="modal-body">
            <input type="hidden" id="id" name="id" value="" readonly>
              <input type="hidden" name="modul" value="category" readonly>
